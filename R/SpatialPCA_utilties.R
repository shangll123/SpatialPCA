...
#' @export
get_NMF = function(object){

	suppressMessages(require(Seurat))
	suppressMessages(require(STutility))
	cat(paste("## Using RunNMF from STUtility package to calculate NMF low dimensional components. \n"))
	seu = CreateSeuratObject(count = object@normalized_expr) 
	seu@assays$RNA@scale.data = object@normalized_expr
	seu@assays$RNA@var.features = rownames(object@normalized_expr)
	seu <- RunNMF(seu, nfactors = object@SpatialPCnum, n.cores = 1, order.by.spcor = FALSE, sort.spcor.by.var = FALSE,rgl.useNULL = TRUE)
...
#' @param object: SpatialPCA object.
#' @return A d by n matrix of low dimensional components from PCA. d is number of low dimensional components, which is same as number of Spatial PCs in SpatialPCA. n is sample size.
#' @export
get_PCA = function(object){
	
	n=dim(object@normalized_expr)[2]
	k = dim(object@normalized_expr)[1]
	output_sub_mean=matrix(0,k,n)
	for(i_k in 1:k){
  		output_sub_mean[i_k,]=object@normalized_expr[i_k,]-mean(object@normalized_expr[i_k,])
	}
	svd_output_sub_mean=svd(output_sub_mean)
	A_ini=svd_output_sub_mean$u[,1:object@SpatialPCnum] 
